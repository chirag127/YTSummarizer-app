...
import { getTTSSettings, saveTTSSettings, getAvailableVoices } from '../services/tts';
import { COLORS, SPACING, FONT_SIZES, TTS_RATE_OPTIONS, TTS_PITCH_OPTIONS } from '../constants';
...
const SettingsScreen = () => {
  ...
  const testTTS = async () => {
    try {
      await Speech.stop();
      await Speech.speak('This is a test of the text-to-speech settings.', {
        rate: settings.rate,
        ...
        <View style={styles.optionsContainer}>
          {TTS_RATE_OPTIONS.map((option) => (
            <TouchableOpacity
              ...
              style={[
                styles.optionButton,
                settings.rate === option.value && styles.optionButtonSelected,
              ]}
            ...
            >
              <Text
                style={[
                  ...
                  settings.rate === option.value && styles.optionButtonTextSelected,
                ]}
              >
                {option.label}
              </Text>
        ...
        <View style={styles.optionsContainer}>
          {TTS_PITCH_OPTIONS.map((option) => (
            <TouchableOpacity
              ...
              style={[
                styles.optionButton,
                settings.pitch === option.value && styles.optionButtonSelected,
              ]}
            ...
            >
              <Text
                style={[
                  styles.optionButtonText,
                  settings.pitch === option.value && styles.optionButtonTextSelected,
                ]}
              >
                {option.label}
              </Text>
              ...
              <View style={styles.voicesContainer}>
                {voicesByLanguage[language].map((voice) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.voiceButton,
                      settings.voice === voice.identifier && styles.voiceButtonSelected,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        ...
                        settings.voice === voice.identifier && styles.voiceButtonTextSelected,
                      ]}
                      numberOfLines={1}
                    >
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.header}>
          <Text style={styles.headerTitle}>Text-to-Speech Settings</Text>
