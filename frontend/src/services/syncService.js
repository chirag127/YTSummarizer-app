...
import { toggleStarSummary, deleteSummary } from './api';
...
export const removeFromSyncLog = async (index) => {
  try {
    ...
    console.error(`Error removing item at index ${index} from sync log:`, error);
    return false;
  }
};
  ...
  try {
    ...
    for (let i = 0; i < syncLog.length; i++) {
      ...
      try {
        switch (entry.action) {
          case 'star':
            await toggleStarSummary(entry.summaryId, true);
            results.push({ success: true, index: i });
            break;
            
          case 'unstar':
            await toggleStarSummary(entry.summaryId, false);
            results.push({ success: true, index: i });
            break;
            
          case 'delete':
            await deleteSummary(entry.summaryId);
            results.push({ success: true, index: i });
            break;
            
          default:
            console.warn(`Unknown action in sync log: ${entry.action}`);
            results.push({ success: false, index: i, error: 'Unknown action' });
        }
      } catch (error) {
        console.error(`Error processing sync log entry at index ${i}:`, error);
